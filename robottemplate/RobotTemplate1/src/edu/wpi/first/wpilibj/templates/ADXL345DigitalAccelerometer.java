 ...
 */
public class ADXL345DigitalAccelerometer extends SensorBase implements PIDSource {
    private I2C i2c;
    // default address
    ...
    double angle = 0;
    double a;
    double angleConstant = 0; //CHANGEPLS
    double someOtherConstant = 0; //CHANGEPLS
    NetworkTable table = NetworkTable.getTable("SmartDashboard");

    public double pidGet() {
        //DO SOMECORRECTIONTHING SUCH AS:
        //RETURN ANGLE - (SOMECONSTANT/ANGLE) + SOMEOTHERCONSTANTIFNEEDED;
        int i = 0;
        angle = 0;
        while (i < 5) {
            a = MathUtils.acos(getXAxis());
            a = a * 180 / Math.PI;
            a = a - (angleConstant/a) + someOtherConstant + table.getNumber("angleOffset");
            angle = angle + a;
            i++;
        }
        return 90- (angle / 5);
